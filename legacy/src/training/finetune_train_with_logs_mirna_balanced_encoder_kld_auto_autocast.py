...
from legacy.src.models.model_1_encode_kld import build_transformer, reparameterize
...
####################################################################################################
config = get_config(config_name='config_kld_finetune_auto')
...
df.iloc[train_indices].to_csv(f'/mnt/tank/scratch/azaikina/Model/data/train_ds_{exp_name}.csv')
df.iloc[test_indices].to_csv(f'/mnt/tank/scratch/azaikina/Model/data/test_ds_{exp_name}.csv')
...
def test_step_autoregressive(model: torch.nn.Transformer, 
              dataloader: torch.utils.data.DataLoader, 
              loss_fn: torch.nn.Module,
              beta: float,
              global_step:int = None,
    ...
    with torch.inference_mode():
        for batch in progress_bar:
            ...
            with torch.autocast(device_type='cuda'):
                mu, logvar = model.encoder(src_input)
                z = reparameterize(mu, logvar)
                z_proj = model.latent_to_model(z).unsqueeze(1)

            input_ids = torch.full((src_input.size(0), 1), tokenizer.sos_id, device=device)
            all_logits = []
            tgt_max_len = batch['decoder_input'].size(1)
            

            for t in range(tgt_max_len):
                with torch.autocast(device_type='cuda'):    
                    tgt_mask = causal_mask(input_ids.size(1)).to(device)
                    decoder_out = model.decode(z_proj, input_ids, tgt_mask)
                    logits = model.project(decoder_out)
                all_logits.append(logits[:, -1:, :])
                next_token = logits[:, -1, :].argmax(dim=-1, keepdim=True)
                input_ids = torch.cat([input_ids, next_token], dim=1)

                # Досрочная остановка для каждого примера в батче
                if torch.all(next_token == tokenizer.eos_id):
                        break

                logits_for_loss = torch.cat(all_logits, dim=1)
                pred_len, target_len = logits_for_loss.size(1), label.size(1)            
                if pred_len < target_len:
                    pad_size = target_len - pred_len
                    pad = torch.zeros((logits_for_loss.size(0), pad_size, logits_for_loss.size(2)), device=device)
                    logits_for_loss = torch.cat([logits_for_loss, pad], dim=1)
                elif pred_len > target_len:
                    pad_size = pred_len - target_len
                    pad = torch.full((label.size(0), pad_size), tokenizer.pad_id, device=device, dtype=label.dtype)
                    label = torch.cat([label, pad], dim=1)
            
                
                recon_loss = loss_fn(logits_for_loss.view(-1, logits_for_loss.size(-1)), label.view(-1))
                kld = kld_loss(mu, logvar)
                beta_kld = beta * kld
                loss = recon_loss + beta_kld


            pred_ids = logits_for_loss.argmax(dim=-1).detach().cpu().numpy()
            model_out_text = [tokenizer.decode_tensors(ids) for ids in pred_ids]
            # Retrieving source and target texts from the batch
            target_text = batch['apt_seq']
            tqdm.write(f'loss {loss}')
...
def train_step_autoregressive(model: torch.nn.Transformer, 
               dataloader: torch.utils.data.DataLoader, 
               loss_fn: torch.nn.Module, 
               optimizer: torch.optim.Optimizer,
               beta: float,
    ...
    for i, batch in progress_bar:
        ...
        target_ids = [tokenizer.encode(seq)[:max_len] for seq in target_text]

        tgt_max_len = max(len(ids) for ids in target_ids)
        target_ids = [ids + [tokenizer.pad_id]*(tgt_max_len - len(ids)) for ids in target_ids]
        target_ids = torch.tensor(target_ids).to(device)  # [batch, tgt_len]
        with torch.autocast(device_type='cuda'):
            mu, logvar = model.encoder(src_input)
            z = reparameterize(mu, logvar)
            z_proj = model.latent_to_model(z).unsqueeze(1)
            
            input_ids = torch.full((config['batch_size'], 1), tokenizer.sos_id, device=device)
            all_logits = []

            for t in range(target_ids.size(1)):
                tgt_mask = causal_mask(input_ids.size(1)).to(device)
                decoder_out = model.decode(z_proj, input_ids, tgt_mask)   # [batch, seq_len, d_model]
                logits = model.project(decoder_out)                       # [batch, seq_len, vocab_size]
                all_logits.append(logits[:, -1:, :])
                next_token = logits[:, -1, :].float().argmax(dim=-1, keepdim=True)
                # Scheduled sampling
                if t < target_ids.size(1):
                    use_teacher = (torch.rand(input_ids.size(0), 1, device=device) < teacher_forcing_ratio)
                    next_input = torch.where(use_teacher, target_ids[:, t:t+1], next_token)
                else:
                    next_input = next_token

                input_ids = torch.cat([input_ids, next_input], dim=1)
                    
            logits_for_loss = torch.cat(all_logits, dim=1)  # [batch, seq_len, vocab]
            pred_len, target_len = logits_for_loss.size(1), target_ids.size(1)
            ...
            if pred_len < target_len:
                ...
                pad_size = pred_len - target_len
                pad = torch.full((target_ids.size(0), pad_size), tokenizer.pad_id, device=device, dtype=target_ids.dtype)
        ...
        pred_ids = logits_for_loss.argmax(dim=-1).detach().cpu().numpy()
        model_out_text = [tokenizer.decode_tensors(ids) for ids in pred_ids]
        for pred_seq, target_seq in zip(model_out_text, target_text):
            pred_seq = clean_sequence(pred_seq)
            target_seq = clean_sequence(target_seq)
            if len(pred_seq) == 0 or len(target_seq) == 0:
                empty_seq_warnings += 1
            ...
            lev_dist = Levenshtein.distance(pred_seq, target_seq)
            total_levenshtein += lev_dist
            total_normalized_lev += lev_dist / len(target_seq)
            count += 1
...
def train(model: torch.nn.Module, 
    ...
    with mlflow.start_run(run_name="Experiment_run"):
        ...
        for epoch in tqdm(range(epochs)):
            train_loss, train_recon, train_kld, train_avg_levenshtein, train_normalized_levenshtein, global_step = train_step_autoregressive(model=model,
                                            dataloader=train_dataloader,
                                            loss_fn=loss_fn,
                                            optimizer=optimizer, global_step= global_step, beta=beta, teacher_forcing_ratio=0.7)
           
            test_loss, test_recon, test_kld, test_avg_levenshtein, test_normalized_levenshtein = test_step_autoregressive(model=model,
