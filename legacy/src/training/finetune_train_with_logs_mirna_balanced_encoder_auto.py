...
####################################################################################################
config = get_config(config_name='config_finetune_encode_auto')
...
def test_step_autoregressive(model: torch.nn.Transformer, 
    ...
    with torch.inference_mode():
        for batch in progress_bar:
            ...
            if encoder_output.dim() == 2:
                encoder_output = encoder_output.unsqueeze(1)
            input_ids = torch.full((encoder_output.size(0), 1), tokenizer.sos_id, device=device)
            all_logits = []
            max_len = batch['decoder_input'].size(1)  # можно брать максимальную длину таргета

            for t in range(max_len):
                tgt_mask = causal_mask(input_ids.size(1)).to(device)
                decoder_out = model.decode(encoder_output, input_ids, tgt_mask)
                logits = model.project(decoder_out)
                all_logits.append(logits[:, -1:, :])
                next_token = logits[:, -1, :].argmax(dim=-1, keepdim=True)
                input_ids = torch.cat([input_ids, next_token], dim=1)

                # досрочная остановка для всех батчей, если все сгенерировали <EOS>
            ...
            # === Вычисление loss ===
            loss = loss_fn(logits_for_loss.view(-1, logits_for_loss.size(-1)), label.view(-1))
            test_loss += loss.item()
            tqdm.write(f'loss {loss}')
            ...
            for pred_seq, target_seq in zip(model_out_text, target_text):
                ...
                if len(mismatch_examples) < 5:
                    mismatch_examples.append(visualize_mismatch(target_seq, pred_seq))
...
def train(model: torch.nn.Module, 
    ...
    with mlflow.start_run(run_name="Experiment_run"):
        ...
        for epoch in tqdm(range(epochs)):
            train_loss, train_avg_levenshtein, train_normalized_levenshtein, global_step = train_step_autoregressive(model=model,
                                            dataloader=train_dataloader,
                                            loss_fn=loss_fn,
                                            optimizer=optimizer, global_step= global_step)
            test_loss, test_avg_levenshtein, test_normalized_levenshtein = test_step_autoregressive(model=model,
