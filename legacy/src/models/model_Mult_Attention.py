...
class MultiplicativeAttention(nn.Module):
    ...
    def forward(self, decoder_embedding, encoder_embedding):
        ...
        encoder_embedding: [batch, 1, d_model]  (sequence embedding)
        """
        proj_enc = self.W(encoder_embedding)  # [batch, 1, d_model]
        scores = torch.bmm(decoder_embedding, proj_enc.transpose(1, 2))   # [batch, tgt_len, 1]
        weights = torch.sigmoid(scores)    # [batch, tgt_len, 1]
        context = weights * proj_enc     # [batch, tgt_len, d_model]
        return context       # [batch, tgt_len, d_model]
...
class DecoderBlock(nn.Module):  
    ...
    def __init__(self,  self_attention_block: MultiHeadAttentionBlock, mult_attention_block: MultiplicativeAttention, feed_forward_block: FeedForwardBlock, dropout: float) -> None:
        super().__init__()
        self.self_attention_block = self_attention_block
        self.mult_attention_block = mult_attention_block
    ...
    def forward(self, x, encoder_output, tgt_mask):
        ...
        # The Multiplicative Attention block using 'encoder_ouput' and 'x' for Decoder queries
        x = self.residual_connections[1](x, lambda x: self.mult_attention_block(x, encoder_output))
...
def build_transformer(tgt_vocab_size: int, tgt_seq_len: int, d_model: int = 512, N: int = 6, h: int = 8, dropout: float = 0.1, d_ff: int = 2048) -> Transformer:
    ...
    for _ in range(N): # Iterating 'N' times to create 'N' DecoderBlocks (N = 6)
        ...
        mult_attention_block = MultiplicativeAttention(d_model) # Multiplicative Attention
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout) # FeedForward
        
        # Combining layers into a DecoderBlock
        decoder_block = DecoderBlock(decoder_self_attention_block, mult_attention_block, feed_forward_block, dropout)
