...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask): 
        ...
        #print("INPUT", q.shape, k.shape, v.shape, flush=True)
...
class DotProductAttention(nn.Module):
    def forward(self, decoder_embedding, encoder_embedding):
        ...
        # encoder_embedding: [batch, 1, d_model]
                # Если encoder_embedding 2D, поднимаем размерность
        if encoder_embedding.dim() == 4 and encoder_embedding.size(1) == 1:
            encoder_embedding = encoder_embedding.squeeze(1)  # [batch, 1, d_model]


        elif encoder_embedding.dim() == 2:
            encoder_embedding = encoder_embedding.unsqueeze(1)  # [batch, 1, d_model]

        elif encoder_embedding.dim() != 3:
            raise ValueError(f"encoder_embedding must be 2D, 3D, or 4D with shape [batch, 1, 1, d_model], got {encoder_embedding.shape}")
...
class Transformer(nn.Module):
    ...
    def __init__(self, encoder: MLPEncoder, decoder: Decoder, tgt_embed: InputEmbeddings, tgt_pos: PositionalEncoding, projection_layer: ProjectionLayer, d_model: int) -> None:
        super().__init__()
        ...
        self.latent_to_model = nn.Linear(self.encoder.latent_dim, d_model)  

    def forward(self, src_input, tgt, tgt_mask, use_kld=False):
        mu, logvar = self.encoder(src_input)
        ...
        z_proj = self.latent_to_model(z).unsqueeze(1)  # [batch, 1, d_model]
        decoder_out = self.decode(z_proj, tgt, tgt_mask)

        out = self.project(decoder_out)
...
def build_transformer(tgt_vocab_size: int, tgt_seq_len: int, d_model: int = 512, N: int = 6, h: int = 8, dropout: float = 0.1, d_ff: int = 2048, encoder_hidden_dim: int = 512, latent_dim: int = 128) -> Transformer:
    ...
    for _ in range(N): # Iterating 'N' times to create 'N' DecoderBlocks (N = 6)
        ...
        decoder_block = DecoderBlock(decoder_self_attention_block, decoder_dot_attention_block, feed_forward_block, dropout)
        decoder_blocks.append(decoder_block) # Appending DecoderBlock to the list of DecoderBlocks
        
    # Creating the Encoder and Decoder by using the EncoderBlocks and DecoderBlocks lists
    mlp_encoder = MLPEncoder(input_dim=d_model, latent_dim=latent_dim, hidden_dim=encoder_hidden_dim)
