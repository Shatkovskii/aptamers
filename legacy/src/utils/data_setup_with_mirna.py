...
from pathlib import Path
from utils import KMerTokenizer

embeddings_path = "/mnt/tank/scratch/azaikina/esm/mirna_embeds"

df_path = '/mnt/tank/scratch/azaikina/Model/RNAInter/final_mirna_dataset.csv'
df = pd.read_csv(df_path, index_col = 0)

#Убрать строки без сиквенсов
df = df.dropna(subset=['sequence_x'])
...
dataset = AptamersDataset(df=df, tokenizer=tokenizer, seq_len=80, embeddings_path = embeddings_path, 
...
train_dataloader_custom = DataLoader(dataset=dataset,
                                     batch_size=5,
                                     shuffle=True,
                                     collate_fn=collate_embeddings)

test_dataloader_custom = DataLoader(dataset=dataset,
                                    batch_size=1,
                                    shuffle=False,
                                    collate_fn=collate_embeddings)
